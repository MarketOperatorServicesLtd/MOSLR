...
render_MPOP_report <- function(
  ...
  my.dir = getwd(),
  load.data = TRUE,
  dir.output = paste0(my.dir, "/MPOP"),
  file.name =
    paste0("MPOPReport-", as.character(format(Sys.Date(), "%Y-%m")), ".pdf"),
...
# Setup -------------------------------------------------------------------
  ...
  if (load.data) {
    ...
    longunread <- utils::read.csv(paste0(my.dir, "/data/inputs/longunread.csv"))%>%
      ...
      dplyr::left_join(
        tp_details, by = c("RetailerID" = "Trading.Party.ID"))%>%
      dplyr::rename(
        Retailer = TradingPartyName, ShortRet = ShortName
        ) %>%
      dplyr::left_join(
        tp_details, by = c("WholesalerID" = "Trading.Party.ID"))%>%
      dplyr::rename(
        Wholesaler = TradingPartyName, ShortWhole = ShortName
      )%>%
      dplyr::mutate(Pair = paste(ShortRet, "+", ShortWhole))


    vacant <- utils::read.csv(paste0(my.dir, "/data/inputs/Vacancy.csv"))%>%
      dplyr::group_by(WholesalerID, Period)%>%
    ...
    vacant_TP <- dplyr::bind_rows(
      read.csv(paste0(my.dir, "/data/inputs/Vacancy.csv"))%>%
        dplyr::group_by(RetailerID, Period)%>%
        dplyr::summarise(Premises = sum(Premises), Vacant_Premises = sum(Vacancies))%>%
    ...
    vacant_Total <- read.csv(paste0(my.dir, "/data/inputs/Vacancy.csv"))%>%
      dplyr::group_by(Period)%>%
      dplyr::summarise(Total_Premises = sum(Premises), Total_Vacant_Premises = sum(Vacancies))%>%
      dplyr::mutate(Period = as.Date(Period, tryFormats = c("%d/%m/%Y", "%Y-%m-%d")), Market_Vacancy_Rate = Total_Vacant_Premises/Total_Premises)
