...
render_monthly_perf_rep <- function(
  ...
  file.name = paste0("MonthlyPerformanceReport-", as.character(format(Sys.Date(), "%Y-%m")), ".pdf")
  ) {
...
# Importing data ----------------------------------------------------------
  ...
  iprp_plan_comparison_mps <- readRDS(dir.iprp.comp)
  ...
  IPRP_plans_mps <- read.csv(iprp.plans) %>%
    mutate(
...
# Importing monthly tracking sheet ----------------------------------------
  ...
  flagged_cols <- c("Trading.Party.ID", "MPS", "Batch", "Action", "Rationale")
