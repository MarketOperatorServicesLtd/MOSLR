...
bubble_chart <- function(
  ...
  if (vacancy.graph) {
    if (prep.data) {
      ...
      main <- dplyr::filter(df, change < 0.15, change > -0.15)
    ...
    graph <-
      ggplot2::ggplot(main, ggplot2::aes(x = percent, y = change, label = ShortName)) +
      ...
      ggplot2::annotate(geom = "label", x = -0.025, y = -max(abs(main$change)), label = "Leading", fill = "azure2") +
      ggplot2::annotate(geom = "label", x = 2 * (vertical.sep + 0.025) + 0.02, y = -max(abs(main$change)), label = "Improving", fill = "azure2") +
      ggplot2::annotate(geom = "label", x = -0.03, y = max(abs(main$change)), label = "At risk", fill = "azure2") +
      ggplot2::annotate(geom = "label", x = 2 * (vertical.sep + 0.025) + 0.023, y = max(abs(main$change)), label = "Declining", fill = "azure2") +
    ...
    if (by == "Wholesaler") {
      title <-
        "Wholesaler area Long Unread Meter change in the last 12 months"
    } else if (by == "Pair")  {
      title <-
        "Long Unread Meter change in the last 12 months per Pairing"
    } else{
      title <- "Retailer Long Unread Meter change in the last 12 months"
    }

    graph <- ggplot2::ggplot(main,
                             ggplot2::aes(x = percent,
                                          y = change,
                                          label = ShortName)) +
      ggplot2::geom_rect(
        xmin = -vertical.sep,
      ...
      ggplot2::geom_point(ggplot2::aes(size = TotalMeters,
                                       fill = `Change in size`),
                          pch = 21) +
      ...
      ggplot2::guides(size = FALSE,
                      fill = ggplot2::guide_legend(override.aes = list(size = 7)))
