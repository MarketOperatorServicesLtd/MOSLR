...
plist_flagged = lapply(split(flagged_mps_data_melt, flagged_mps_data_melt$key), function(z) {
  ...
  plot <- ggplot(data) + 
    ...
    scale_y_continuous(
      sec.axis = sec_axis(~ . / max(data$TaskVolume), name = "Proportion")
      ) +
    scale_fill_manual(
    ...
    ggtitle(paste(Trading.Party.ID, " (", MPS, ")", sep = ""))
...
plist_watch = lapply(split(watch_mps_melt, watch_mps_melt$key), function(data) {
  ...
  plot <- ggplot(data) + 
    ...
    ggtitle(paste0(data$Trading.Party.ID,"  (", data$MPS, ")"))
...
plist_mps_agg = lapply(split(mps_summary_melt, mps_summary_melt$MPS), function(data) {
  ...
  ggplot(data) +
    ...
    scale_y_continuous(
      sec.axis = sec_axis(~. / max(data$TotalTaskVolume), name = "Proportion")
      ) +
    scale_fill_manual(
      NULL, values = "azure3"
      ) +
