...
`r SHORT.NAME`'s on-time task performance for `r MPS_CHUNK` is rated as:
...
* `r perf_status_mps_temp$SumPerf3m[perf_status_mps_temp$Standard == MPS_CHUNK]` performance flags
* `r perf_status_mps_temp$CumWatch[perf_status_mps_temp$Standard == MPS_CHUNK]` watch flags
...
perf_status_mps %>%
  ...
  dplyr::select(
    Date, OnTimeTaskCompletion, Planned_Perf, DeltaQuant, Status
    ) %>%
  tidyr::drop_na() %>%
