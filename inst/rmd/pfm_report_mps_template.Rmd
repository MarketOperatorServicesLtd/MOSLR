...
Below is a graph showing `r SHORT.NAME`'s historical performance against `r MPS_CHUNK`. MOSL action points and IPRP milestones are shown, if applicable. 

```{r message = FALSE, echo = FALSE, error = FALSE, warning = FALSE, fig.width = 12, fig.height = 5}

on.iprp <- perf_status_mps_temp$IPRP[perf_status_mps_temp$Standard == MPS_CHUNK]
...
perf_status_mps %>%
  ...
  kableExtra::kable(
    ...
    longtable = TRUE, 
    booktabs = TRUE,
    col.names = c("Date", "Action", "Rationale")
    ) %>%
...
perf_status_mps %>%
  ...
  dplyr::select(
    Date, Performance, Planned_Perf, DeltaQuant, Status
    ) %>%
  tidyr::drop_na() %>%
  ...
  kableExtra::kable(
    ...
    longtable = TRUE, 
    booktabs = TRUE,
    col.names = c("Date", "Actual Performance", "Planned Performance", "Relative Difference", "IPRP Status"),
    align = rep("c", 5)
    ) %>%
...
`r if (on.iprp) { paste0("* ",  perf_status_mps_temp$CumResubmit[perf_status_mps_temp$Standard == MPS_CHUNK], " resubmission(s)") }`
`r if (on.iprp) { paste0("* ",  sum(perf_status_mps_temp$UnderReview[perf_status_mps_temp$Standard == MPS_CHUNK]), " review(s)") }`
